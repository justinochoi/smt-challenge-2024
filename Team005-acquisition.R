...
# Load in game info data 
game_info_83 <- game_info %>%
  filter(substr(game_str, 1, 4) == "1883") %>%
  collect() 
game_info_84 <- game_info %>%
  filter(substr(game_str, 1, 4) == "1884") %>%
  collect()  
game_info_df <- bind_rows(game_info_83, game_info_84)

# Info on shortstops from the home team, to be used later 
ss_info <- game_info_df %>% 
  filter(!is.na(shortstop), top_bottom == "top") %>% 
  select(game_str, play_per_game, shortstop) %>% 
...
ss_tracking <- grounder_events %>% 
  filter(event_code == 2) %>% 
  select(game_str, play_id, timestamp_on_contact, 
         timestamp_on_acquire, ss_flag, ss_got_ball, farm_level) %>% 
  left_join(select(ball_pos_df, game_str, play_id,  
...
# We don't care about ball position beyond a certain point
...
# We want to know which shortstop was involved in each play 
# Shortstops not part of the home team will be NA
ss_tracking <- ss_tracking %>% 
  left_join(ss_info, by = c("game_str", "play_id")) 
...
ss_metrics_one <- ss_tracking %>% 
  ...
  select(game_str, play_id, farm_level, shortstop, 
         euclid_dist, angle_between, ss_got_ball) %>%
...
# Creating a final table to use for modeling and analysis  
final_df <- ss_metrics_two %>% 
  left_join(ss_metrics_one, by = c("game_str", "play_id")) %>%
  select(game_str, play_id, farm_level, shortstop, euclid_dist, angle_between, 
